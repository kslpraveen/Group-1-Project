...
public static void psnmDBLP(File file,String skey,int window){
	try{
		dataset.clear();
		duplicates.clear();
		long tot = 0;
...
public static void psnmCD(File file,String skey,int window){
	try{
		dataset.clear();
		duplicates.clear();
		long tot = 0;
			...
			for(String value : arr){
				...
				if(columns.length > 4){
					if(skey.equals("title")){
						columns[3] = columns[3].replaceAll("[^a-zA-Z\\s+]", "");
						Sort sr = new Sort();
						sr.setTitle(columns[3].trim());
						dataset.add(sr);
						order = order + 1;
					}
					if(skey.equals("artist")){
						columns[2] = columns[2].replaceAll("[^a-zA-Z\\s+]", "");
						Sort sr = new Sort();
						sr.setTitle(columns[2].trim());
						dataset.add(sr);
						order = order + 1;
					}
					if(skey.equals("category")){
						columns[4] = columns[4].replaceAll("[^a-zA-Z\\s+]", "");
						Sort sr = new Sort();
						sr.setTitle(columns[4].trim());
						dataset.add(sr);
						order = order + 1;
					}
				}
