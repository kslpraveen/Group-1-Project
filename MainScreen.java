...
public MainScreen(){
	...
	b1.addActionListener(new ActionListener(){
		public void actionPerformed(ActionEvent ae){
			...
			if(option == chooser.APPROVE_OPTION){
				...
				dataset_name = file.getName().trim();
				c1.removeAllItems();
				if(dataset_name.equals("dblp.xml")){
					c1.addItem("title");
					c1.addItem("author");
				}
				if(dataset_name.equals("cd.csv")){
					c1.addItem("title");
					c1.addItem("artist");
					c1.addItem("category");
				}
		...
		public void actionPerformed(ActionEvent ae){
			...
			new Thread(new Runnable(){
				public void run(){
					...
					if(dataset_name.equals("dblp.xml")){
						PSNM.psnmDBLP(file,c1.getSelectedItem().toString().trim(),Integer.parseInt(c2.getSelectedItem().toString().trim()));
					}
					if(dataset_name.equals("cd.csv")){
						PSNM.psnmCD(file,c1.getSelectedItem().toString().trim(),Integer.parseInt(c2.getSelectedItem().toString().trim()));
					}
					Cursor normalCursor = new Cursor(Cursor.DEFAULT_CURSOR);
					setCursor(normalCursor);
					ArrayList<String> dup = new ArrayList<String>(PSNM.duplicates);
					...
					System.out.println("Normal Processing Time : "+(end-start));
				}
			}).start();
		}
	});
		...
		public void actionPerformed(ActionEvent ae){
			...
			new Thread(new Runnable(){
				public void run(){
					...
					start = System.currentTimeMillis();
					if(dataset_name.equals("dblp.xml")){
						PB.pbDBLP(file,c1.getSelectedItem().toString().trim(),Integer.parseInt(c2.getSelectedItem().toString().trim()));
					}
					if(dataset_name.equals("cd.csv")){
						PB.pbCD(file,c1.getSelectedItem().toString().trim(),Integer.parseInt(c2.getSelectedItem().toString().trim()));
					}
					end = System.currentTimeMillis();
					Cursor normalCursor = new Cursor(Cursor.DEFAULT_CURSOR);
					setCursor(normalCursor);
	...
	b7 = new JButton("Parallel");
	b7.setFont(f2);
	b7.setBounds(640,160,150,30);
	p2.add(b7);
		...
		public void actionPerformed(ActionEvent ae){
			...
			new Thread(new Runnable(){
				public void run(){
					...
					Parallel screen = new Parallel(file,dataset_name,c1.getSelectedItem().toString().trim(),Integer.parseInt(c2.getSelectedItem().toString().trim()));
					screen.startRunning();
				}
			}).start();
		}
	...
	b5.setFont(f2);
	b5.setBounds(800,160,220,30);
	p2.add(b5);
	b5.addActionListener(new ActionListener(){
		public void actionPerformed(ActionEvent ae){
			Comparision chart1 = new Comparision("Comparision Chart",propose_time,(end-start));
			chart1.pack();
			RefineryUtilities.centerFrameOnScreen(chart1);
			chart1.setVisible(true);
		}
